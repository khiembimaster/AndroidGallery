...
public class _PhotoSyncWorker extends Worker {
    ...
    public _PhotoSyncWorker(@NonNull Context context, @NonNull WorkerParameters workerParams) {
    ...
    public Result doWork() {
        ...
        try {
            ...
            if (response != null) {
                ...
                HashMap<String, PhotoDetailsInLocal> remotePhotosMap = new HashMap<>(
                        (int) Math.ceil(remotePhotos.size() / 0.75)
                );
                for (PhotoDetails remotePhoto : remotePhotos) {
                    remotePhotosMap.put(remotePhoto.localPath,
            ...
            for (Photo photo : photosToUpload) {
                ...
                task.addOnSuccessListener(uri -> {
                    ...
                    _PhotoSyncWorker.this.setProgressAsync(new Data.Builder()
                            .putLong(TOTAL_COUNT_KEY, --count)
                            .build());
    ...
    private Task<Void> downloadImage(PhotoDetails photoDetails) {
        // Glide.with(getApplicationContext())
        //         .asBitmap()
        //         .load(photoDetails.remoteUrl)
        //         .into(new CustomTarget<Bitmap>() {
        //             @Override
        ...
        //             ) {
        //                 Path path = Paths.get(photoDetails.localPath);
        //                 Path parent = path.getParent();
        //
        //
        //                 if (parent == null) return;
        //
        //                 File folder = new File(parent.toString());
        //                 if (!folder.exists()) {
        //                     boolean created = folder.mkdirs();
        //                     if (!created) return;
        //                 } else {
        //                     createUniqueFolder(Environment.DIRECTORY_PICTURES,
        //                             parent.getFileName().toString());
        //                 }
        //
        //                 DownloadManager.Request request =
        //                         new DownloadManager.Request(Uri.parse(photoDetails.remoteUrl));
        //                 request.allowScanningByMediaScanner();
        //                 request.setTitle("Syncing photos")
        //             }
        //
        //             @Override
        //             public void onLoadCleared(@Nullable Drawable placeholder) {
    ...
    private File createUniqueFolder(String baseDir, String folderName) {
        ...
        while (folder.exists()) {
            ...
            folder = new File(baseDir, newFolderName);
            counter++;
        }
        ...
        if (folder.mkdirs()) {
            ...
            return null; // Return null if unable to create the folder
        }
    }
