...
//
//        while (true) {
//            if (mNoMoreFrames && mEncodeQueue.isEmpty()) break;
//
...
//                    inputBuffer.clear();
//                    if (byteConvertFrame.length <= inputBuffer.remaining()) {
//                        inputBuffer.put(byteConvertFrame);
//                        mediaCodec.queueInputBuffer(inputBufIndex, 0, byteConvertFrame.length, ptsUsec, 0);
//                        mGenerateIndex++;
//                    } else {
//                        // Handle overflow buffer by splitting the frame
//                        Log.e(TAG, "Input frame size exceeds input buffer capacity");
//                        List<byte[]> frameSegments = splitFrame(byteConvertFrame);
//                        for (byte[] segment : frameSegments) {
//                            encodeSegment(segment);
//                        }
//                    }
//                } else {
//                    // Handle when unable to dequeue input buffer
//                    Log.e(TAG, "Failed to dequeue input buffer");
//                }
//            } catch (Exception ex) {
...
//        }
//    }
//    private void encodeSegment(byte[] segment) {
//        try {
//            long TIMEOUT_USEC = 500000;
//
//            // Dequeue an input buffer from the encoder
//            int inputBufIndex = mediaCodec.dequeueInputBuffer(TIMEOUT_USEC);
//            if (inputBufIndex >= 0) {
//                // Get the input buffer and clear it
//                final ByteBuffer inputBuffer = mediaCodec.getInputBuffer(inputBufIndex);
//                inputBuffer.clear();
//
//                // Put the segment data into the input buffer
...
//                mediaCodec.queueInputBuffer(inputBufIndex, 0, segment.length, ptsUsec, 0);
//                mGenerateIndex++;
//            } else {
//                // Handle when unable to dequeue input buffer
//                Log.e(TAG, "Failed to dequeue input buffer");
//            }
//
//            // Dequeue and process the output buffer
//            MediaCodec.BufferInfo mBufferInfo = new MediaCodec.BufferInfo();
//            int encoderStatus = mediaCodec.dequeueOutputBuffer(mBufferInfo, TIMEOUT_USEC);
//            if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
//                Log.e(TAG, "No output from encoder available");
...
//    }
//
//    private List<byte[]> splitFrame(byte[] frameData) {
//        // Split the frame into smaller segments (adjust the segment size as needed)
//        List<byte[]> segments = new ArrayList<>();
//        int segmentSize = 1024; // Adjust the segment size based on your requirements
//        for (int i = 0; i < frameData.length; i += segmentSize) {
//            int length = Math.min(segmentSize, frameData.length - i);
//            byte[] segment = Arrays.copyOfRange(frameData, i, i + length);
//            segments.add(segment);
//        }
//        return segments;
//    }
//    private int getNumberOfChannels(MediaFormat format) {
//        if (format.containsKey(MediaFormat.KEY_CHANNEL_COUNT)) {
//            return format.getInteger(MediaFormat.KEY_CHANNEL_COUNT);
//        } else {
//            return 0; // Return default value or handle the absence of channel count as per your requirements
//        }
//    }
//    private int getMaxWidthForMimeType(String mimeType) {
//        int maxWidth = -1;
//
//        // Get the list of supported codecs for the given MIME type
//        MediaCodecList codecList = new MediaCodecList(MediaCodecList.REGULAR_CODECS);
//        MediaCodecInfo[] codecInfos = codecList.getCodecInfos();
//
//        for (MediaCodecInfo codecInfo : codecInfos) {
//            // Check if the codec supports encoding for the specified MIME type
//            if (Arrays.asList(codecInfo.getSupportedTypes()).contains(mimeType)) {
//                // Get the codec capabilities for the specified MIME type
...
//        return maxWidth;
//    }
//    private int getMaxHeightForMimeType(String mimeType) {
//        int maxHeight = -1;
//
//        // Get the list of supported codecs for the given MIME type
//        MediaCodecList codecList = new MediaCodecList(MediaCodecList.REGULAR_CODECS);
...
//
//
//    private byte[] resizeFrameToFitBuffer(Bitmap bitmap, int maxWidth, int maxHeight) {
//        // Calculate the aspect ratio of the original bitmap
//        float originalWidth = bitmap.getWidth();
//        float originalHeight = bitmap.getHeight();
//        float aspectRatio = originalWidth / originalHeight;
//
//        // Calculate the target width and height based on the maximum dimensions
//        int targetWidth = Math.min(maxWidth, (int) (maxHeight * aspectRatio));
//        int targetHeight = Math.min(maxHeight, (int) (maxWidth / aspectRatio));
//
//        // Resize the bitmap to the target dimensions
//        Bitmap resizedBitmap = Bitmap.createScaledBitmap(bitmap, targetWidth, targetHeight, true);
//
//        // Convert the resized bitmap to NV21 format
//        return getNV21(resizedBitmap.getWidth(), resizedBitmap.getHeight(), resizedBitmap);
//    }
//
//
